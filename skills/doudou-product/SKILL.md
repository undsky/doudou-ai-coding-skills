...
资料可以来自知识库、本地需求目录、一个可访问的参考项目链接，或两两结合。先根据用户已提供的信息选择路径。

- **未指定资料来源与参考链接**：默认通过 `doudou-wiki` 知识库（项目根目录下的 `kb/`）读取需求资料（优先读取 `kb/wiki/` 结构化知识页面及 `kb/gen/` 中间产物文本），按“先收集资料”处理。读取前先告知用户将从知识库取材，不要静默改用其他目录。
- **仅知识库/本地目录**：按“先收集资料”读取资料，完成既有五份产品设计交付物。
...
- **知识库/本地目录 + 参考链接**：知识库与本地资料用于确认业务目标、约束与实现现状，参考项目用于补足体验、功能、流程和交互细节。明确区分“需求已确认”“参考项目观察”“设计推断”三类信息，不能把参考项目的行为误写成用户已确认需求。
...
- 两者均未提供时，默认使用项目根目录下的 `kb/` 知识库作为资料来源，优先阅读 `kb/wiki/` 中的结构化知识页面（实体、概念、概述、分析等）与 `kb/gen/` 中的中间产物文本，先列出其中的文件并告知用户取材位置，再按步骤读取。项目根从当前工作目录向上查找，遇到 `.agents`、`.claude`、`.git` 或 `kb/` 即认作项目根。
- 默认知识库不存在、为空或没有可用需求资料时，不要自行臆造资料，也不要静默改用其他无关目录，使用交互式问题询问：

  > `kb/` 知识库中没有可用的需求资料。请提供本次需求资料所在的目录路径、参考项目链接，或使用 `doudou-wiki` 技能（将素材放入 `kb/raw/` 后执行 `/doudou-wiki add`，或直接执行 `/doudou-wiki add <文件路径>`）收录需求资料后再继续。

- 只提供参考链接且目标是分析/借鉴/梳理参考项目时，直接进入“参考项目链接分析”；不要为尚未要求的数据库 SQL 预先询问数据库类型或命名规范。
- 使用默认 `kb/` 知识库或提供本地目录时：
  1. 列出其中的文件，优先阅读需求说明、现有 PRD、原型说明、接口文档、数据库 DDL、项目中的实体模型和相关业务代码；使用知识库时优先阅读 `kb/wiki/` 的 overview、entity、concept 及需求相关页面，必要时对照 `kb/gen/` 中的原始文本化产物。
  2. 识别资料格式与覆盖范围。对图片、PDF、表格、音视频或原型链接，非纯文本素材可借助 `doudou-wiki` 技能及其联动技能（`doudou-ocr` / `doudou-doc` / `doudou-stt`）先处理为 `kb/gen/` 文本再收录；无法直接读取时说明限制并请用户补充关键内容。当同一来源存在多个版本时，以 `updated` 时间戳最新的一份为准，并在汇报中说明所选版本与引用页面。
...
- 资料取自哪个知识库或目录（使用默认知识库 `kb/` 时须明确说明引用的 wiki 页面与素材），已读取的核心资料，以及已分析的参考链接和可访问范围（适用时）；
